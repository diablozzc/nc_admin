 ...
 */
  ...
  app.factory('ucauth', function (Models,$q,$state,$window,notify,config) {
    ...
    function signIn(username,password){
      var deferred = $q.defer();
      var the_data = {'user':username,'password':password};
      

      Models.init('Usertokens').actions('signin',the_data).then(function(ret){
        if(ret.meta.code === 200){
          $window.sessionStorage.tokenSecret = ret.data.tokenSecret;
          $window.sessionStorage.userToken = ret.data.userToken;

          var user_info = _formatUserFrom(ret.data.userToken);
