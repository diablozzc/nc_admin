...
/**
 * @ngdoc directive
 * @name propertyAdminApp.directive:tableDolly
 * @description
 * # tableDolly
 */
...
(function ($) {
  ...
  app.directive('tableDolly', function ($timeout) {
    return {
      scope:{
        tableData:'='
      },
      template: '<div><table class="dolly">' +
      '<tr ng-repeat="row in data">' +
        '<td class="room" ng-repeat="col in row.level" ng-class="{\'selected\':col.select}">{{col.roomNumber}}</td>' +
      '</tr>' +
      '</table>' +
      '</div>',
      restrict: 'A',
      link: function(scope, element, attrs) {
        ...
        scope.onClone = function(el, cloneX, cloneY, indexX, indexY){
          //console.log(el, cloneX, cloneY, indexX, indexY);

            var dirX = cloneX > 0 ? 1 : -1,
            dirY = cloneY > 0 ? 1 : -1,
            amountX = Math.abs(cloneX),
            amountY = Math.abs(cloneY);

          for(var i = indexY; amountY >= 0; i += dirY, amountY--){
            var amX = scope.data[i].level.length-1 < amountX ? scope.data[i].level.length-1 : amountX;
            //var amX = amountX;
            for(var j = indexX; amX >= 0; j += dirX, amX--){
              if(angular.isDefined(scope.data[i].level[j])){
                scope.data[i].level[j].select = !scope.data[i].level[j].select;
              }
            }
          }
       ...
       scope.$on('create_building',function(e,data){
         scope.data = data;
         console.log(data);
         ...
         $timeout(function(){
           ...
           $("td.room").dolly({
             ...
             cloned: function (event, ui) {
               scope.onClone(this, ui.cloneX, ui.cloneY, ui.originX, ui.originY);
               scope.$emit('onBuildingDataUpdate',scope.data);
