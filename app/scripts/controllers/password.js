 ...
 */
  ...
  app.controller('PasswordCtrl', function ($scope,$state,ucauth,Models,notify) {
    ...
    $scope.updateLoginPassword = function(){
      ...
      $scope.loginPassword.oldPassword = md5($scope.loginPassword.oldPassword);
      $scope.loginPassword.newPassword = md5($scope.loginPassword.newPassword);

      Models.init('Passwords/Update').actions('update',$scope.loginPassword).then(function(ret){
        if(ret.meta.code == 200){
